...
for probeId in probe:
  for timeId in probe.get(probeId):
    ...
    print '%s\t%s\t%s\thost_compute\th\t%s\t%s\t%s\t%s\t%s'% (probeId, (probeId.split('_'))[0], hostId, datetime.datetime.fromtimestamp(float(sysMin)+float(timeId)*3600), str(av_UM), str(av_FS), str(av_CL), probe.get(probeId).get(timeId).get('Chostname'))
